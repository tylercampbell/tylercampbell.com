// Client logic for the Postcard component (migrated from src/assets/global.js)
export function initPostcard() {
  ...
  function enableSubmitButton() {
    if (!submitButton || !messageInput || !senderInput) return;
  ...
  function setSenderInputWidth() {
    ...
    const width = Math.floor(getTextWidth(senderInput)) + 30;
  ...
  function setMessageTextareaHeight(e) {
    ...
    if (textarea.classList.contains('autoExpand') && textarea.nodeName === 'TEXTAREA') {
      const rows = parseInt(textarea.getAttribute('data-min-rows') || '3', 10);
      textarea.rows = rows;
      const baseScrollHeight = textarea._baseScrollHeight || getBaseScrollHeight(textarea);
      const currentScrollHeight = textarea.scrollHeight;
      textarea.rows =
        currentScrollHeight < baseScrollHeight
          ? rows
          : rows + Math.ceil((currentScrollHeight - baseScrollHeight) / 16);
    }
  }

  formElement.addEventListener('input', (e) => {
    const target = e.target;
    if (target === messageInput || target === senderInput) {
      enableSubmitButton();
      if (target === senderInput) {
        setSenderInputWidth();
      } else {
  ...
  function handleFormSubmissionSuccess() {
    ...
    if (successElement) successElement.classList.replace('hidden', 'grid');
  }
